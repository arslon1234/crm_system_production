...
export default function FakturaId2({ keyword, url }) {
  ...
  useEffect(() => {
    ...
    if (keyword.length > 0) {
      ...
      timer.current = setTimeout(() => {
        ...
        api
          .get(`finance/income/?to_where=${urlId[6]}`, {
            params: {
              search: keyword.length > 0 ? keyword : "",
            },
          })
      ...
      api.get(`finance/income/?to_where=${urlId[6]}`).then((res) => {
        if (mounted) {
          setLoading(false);
          setData2(res.results);
          setNext2(res.next);
  ...
  return (
    <div>
      <div className="faktura_name" style={{ display: "flex" }}>
        <NavLink activeClassName="navbar_active2" to={`${url}/QarzHaq`}>
